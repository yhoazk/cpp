...
public:
    ...
    char* allocate(std::size_t n){
        assert(pointer_in_buffer(ptr_) && "stack_allocator has outlived stack store");
        n = align(n);
        if(buf_ + N >= ptr_ + n){
            auto r(ptr_);
            ptr_ += n;
    ...
    void deallocate(char* const p, std::size_t n) noexcept {
        assert(pointer_in_buffer(ptr_) && "Stack allocator outlived stack store");
...
namespace std {
    ...
    template<class CharT, class Trairs, class Allocator> class basic_string;
...
using stack_unordered_map = ::std::unordered_map<Key, T, Hash, Pred, stack_allocator<::std::pair<Key const T>, 256>>;
